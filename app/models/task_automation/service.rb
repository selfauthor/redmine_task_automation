...
module TaskAutomation
	class Service
		...
		def self.check_fields_available_for_tracker(tracker_id)
			...
			REQUIRED_CUSTOM_FIELDS.each do |field_name|
				custom_field = CustomField.find_by(name: field_name)
		...
		# ============================================================================
		# Метод проверки прав автора на создание задач в проекте
		# ============================================================================
		def self.check_author_permissions(author_id, project_id)
			...
			unless member
				...
				author.groups.each do |group|
					...
					if group_member && group_member.roles.any? { |role| role.permissions.include?(:add_issues) }
						has_permission = true
						break
					end
				end
				...
				unless has_permission
					issues << {
						type: 'error',
						message: "Пользователь #{author.login} не имеет прав на создание задач в проекте #{project.name}"
					}
				end
			else
				unless member.roles.any? { |role| role.permissions.include?(:add_issues) }
					issues << {
						type: 'error',
						message: "Пользователь #{author.login} не имеет права 'add_issues' в проекте #{project.name}"
					}
				end
			end
